...
export default function FindSpotPage() {
  ...
  useEffect(() => {
    const fetchFavorites = async () => {
      ...
      setFavorites(data.map((spot: { api_place_id: string }) => spot.api_place_id));
      } catch (err) {
        console.error(err);
      }
    };
  ...
  const toggleFavorite = async (spot: SpotData) => {
    ...
    try {
      ...
      const data = await res.json();
      setFavorites(data.map((f: { api_place_id: string }) => f.api_place_id));
    } catch (err) {
  ...
  if (err instanceof Error) {
    ...
    setError("Error al buscar en la API");
  }
  setSearchedSpot(null);
      } finally {
        setLoading(false);
